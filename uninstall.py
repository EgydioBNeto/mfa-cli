...
This script is used to uninstall the MFA CLI tool. It removes the installation directory,
deletes any configuration files related to the tool, and cleans up the environment.

Usage:
    python uninstall.py

Note: After uninstallation, it is recommended to restart the terminal for the aliases to take effect.
...
def uninstall_mfa_cli():
    """
    Uninstall the MFA CLI tool.

    This function removes the installation directory, deletes any configuration files related to the tool,
...
def main():
    ...
    This function calls the uninstall_mfa_cli() function.
    """
    uninstall_mfa_cli()
